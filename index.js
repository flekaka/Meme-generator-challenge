...
const parentNode = document.getElementById('node').parentNode;
...
*/
function clearAll() {
  const existingData = parentNode.querySelectorAll('p, img');
  
  existingData.forEach(generatedElement => { 
    parentNode.removeChild(generatedElement)
  });
}
...
lastButtonClicked = 'meme';
clearAll();
const imgElement = document.createElement('img');
  
  // Value is a string representing image URL
  const randomMemeUrl = getRandomData('memes');
  imgElement.setAttribute('src',randomMemeUrl);
  parentNode.insertBefore(imgElement,node1);
...
const jokeElement = document.createElement('p');
  ...
  parentNode.insertBefore(jokeElement,node2);
...
*/
function showQuote() {
lastButtonClicked = 'quote';
clearAll();
const quoteElementQ = document.createElement('p');
const quoteElementA = document.createElement('p');

  // Value should be in format: { quote: '', author: '' }
  const randomQuote = getRandomData('quotes');
  
  quoteElementQ.textContent = randomQuote.quote; 
  parentNode.insertBefore(quoteElementA,node3); 
  quoteElementA.textContent = '  - ' 
  + randomQuote.author;
  parentNode.insertBefore(quoteElementQ,node3.previousSibling); 
...
function showRiddle() {
lastButtonClicked = 'riddle';
clearAll();
  
  // Value should be in format: { question: '', answer: '' }
  const randomRiddle = getRandomData('riddles');
  
  riddleElement = document.createElement('p');
  riddleElement.textContent = randomRiddle.question;
  riddleElement.dataset.riddleAnswer = randomRiddle.answer;
  
  parentNode.insertBefore(riddleElement,node3.nextSibling);
}

/**
* TODO: Unhide the riddle's answer
* - If there is no riddle shown, alert the user that there is no riddle DONE 
* - If there is a riddle shown and an answer shown, alert the user
*   that the answer is already revealed DONE
* - If there is a riddle shown but no answer, unhide the answer!
*/

function revealAnswers() {
  
if (riddleElement.getAttribute('data-riddle-answer') === 'undefined') {
  alert('There is no answer');
 } 
else if (lastButtonClicked === 'answer') {
  alert('There is aleady an answer shown');
}
else if (lastButtonClicked !== 'riddle') {
  alert('There is no riddle shown');
 } 
else if (lastButtonClicked === 'riddle') {
  answerElement = document.createElement('p');
  answerElement.textContent = riddleElement.dataset.riddleAnswer;
  parentNode.insertBefore(answerElement,riddleElement.nextSibling);
  lastButtonClicked = 'answer'; 
} 
...
// Source: https://www.rd.com/list/challenging-riddles/
 ...
 const riddles = [
  ...
  { question: 'What word in the English language does the following: the first two letters signify a male, the first three letters signify a female, the first four letters signify a great, while the entire world signifies a great woman. What is the word?', answer: 'Heroine' },
   { question: 'Should return alert if you look for the answer'}
];
